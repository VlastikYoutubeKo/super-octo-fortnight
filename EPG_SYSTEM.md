...
# → {"mapping_total":6353,"mapping_deleted":0,"mapping_repointed":0,
#    "tvh_channels":3552,"tvh_relinked":0,"tvh_cleared":0,
#    "epg_universe":17937,"duration_seconds":41}
```
...
2. **Sanitize `epg_mapping.json`**: delete/re-point entries that are dangling
   (id not loaded), name a more-specific different channel (`ExtraIDWords` > 0),
   or belong to PPV/VOD/fixture listings. Writes `epg_mapping.json.pre-maintenance`
   backup before any change.
3. **Relink TVHeadend**: fetch the playlists each TVH network consumes (self-request
...
4. **Clear garbage links** on channels with no tvg-id whose name-matched link shares
   zero distinguishing words (`STEVEN SEGAL`→Playboy class). `epgauto: false` on all
   touched channels.

The run is **idempotent** — a second run right after reports all zeros.
...
| Force one TVH channel's EPG | `POST /api/idnode/save` form-field `node=[{"uuid":"<channel uuid>","epggrab":["<epg source uuid>"],"epgauto":false}]` |
